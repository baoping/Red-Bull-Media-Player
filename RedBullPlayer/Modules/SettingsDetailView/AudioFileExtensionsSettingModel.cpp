...
AudioFileExtensionsSettingModel::AudioFileExtensionsSettingModel( QObject *parent, ISettingsFileExtensionsSettingView *view )
    : ISettingsModel( parent ) {
    _view = view;
    _originalSetting = ConfigManager::instance()->getAudioWhiteList();
    _currentSetting = _originalSetting;
    _view->setValue( _originalSetting );
}

void AudioFileExtensionsSettingModel::saveData() {
    _currentSetting = _view->getValue();
    ConfigManager::instance()->setAudioWhiteList( _currentSetting );
