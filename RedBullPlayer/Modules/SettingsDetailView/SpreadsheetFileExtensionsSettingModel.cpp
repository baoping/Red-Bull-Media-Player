...
SpreadsheetFileExtensionsSettingModel::SpreadsheetFileExtensionsSettingModel( QObject *parent, ISettingsFileExtensionsSettingView *view )
    : ISettingsModel( parent ) {
    _view = view;
    _originalSetting = ConfigManager::instance()->getExcelDocuemntWhiteList();
    _currentSetting = _originalSetting;
    _view->setValue( _originalSetting );
}

void SpreadsheetFileExtensionsSettingModel::saveData() {
    _currentSetting = _view->getValue();
    ConfigManager::instance()->setExcelDocumentWhiteList( _currentSetting );
