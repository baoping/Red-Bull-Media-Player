...
OthersFileExtensionsSettingModel::OthersFileExtensionsSettingModel( QObject *parent, ISettingsFileExtensionsSettingView *view )
    : ISettingsModel( parent ) {
    _view = view;
    _originalSetting = ConfigManager::instance()->getOtherDocuemntWhiteList();
    _currentSetting = _originalSetting;
    _view->setValue( _originalSetting );
}

void OthersFileExtensionsSettingModel::saveData() {
    _currentSetting = _view->getValue();
    ConfigManager::instance()->setOtherDocumentWhiteList( _currentSetting );
