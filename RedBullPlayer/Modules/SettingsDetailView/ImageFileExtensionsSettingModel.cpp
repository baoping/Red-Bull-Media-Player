...
ImageFileExtensionsSettingModel::ImageFileExtensionsSettingModel( QObject *parent, ISettingsFileExtensionsSettingView *view )
    : ISettingsModel( parent ) {
    _view = view;
    _originalSetting = ConfigManager::instance()->getImageWhiteList();
    _currentSetting = _originalSetting;
    _view->setValue( _originalSetting );
}

void ImageFileExtensionsSettingModel::saveData() {
    _currentSetting = _view->getValue();
    ConfigManager::instance()->setImageWhiteList( _currentSetting );
