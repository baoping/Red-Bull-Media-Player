...
VideoFileExtensionsSettingModel::VideoFileExtensionsSettingModel( QObject *parent, ISettingsFileExtensionsSettingView *view )
    : ISettingsModel( parent ) {
    _view = view;
    _originalSetting = ConfigManager::instance()->getVideoWhiteList();
    _currentSetting = _originalSetting;
    _view->setValue( _originalSetting );
}

void VideoFileExtensionsSettingModel::saveData() {
    _currentSetting = _view->getValue();
    ConfigManager::instance()->setVideoWhiteList( _currentSetting );
