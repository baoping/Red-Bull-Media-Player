...
PresentationFileExtensionsSettingModel::PresentationFileExtensionsSettingModel( QObject *parent, ISettingsFileExtensionsSettingView *view )
    : ISettingsModel( parent ) {
    _view = view;
    _originalSetting = ConfigManager::instance()->getPowerpointDocumentWhiteList();
    _currentSetting = _originalSetting;
    _view->setValue( _originalSetting );
}

void PresentationFileExtensionsSettingModel::saveData() {
    _currentSetting = _view->getValue();
    ConfigManager::instance()->setPowerpointDocuemntWhiteList( _currentSetting );
