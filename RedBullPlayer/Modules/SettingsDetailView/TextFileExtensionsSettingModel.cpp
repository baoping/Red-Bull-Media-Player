...
TextFileExtensionsSettingModel::TextFileExtensionsSettingModel( QObject *parent, ISettingsFileExtensionsSettingView *view )
    : ISettingsModel( parent ) {
    _view = view;
    _originalSetting = ConfigManager::instance()->getWordDocumentWhiteList();
    _currentSetting = _originalSetting;
    _view->setValue( _originalSetting );
}

void TextFileExtensionsSettingModel::saveData() {
    _currentSetting = _view->getValue();
    ConfigManager::instance()->setWordDocumetWhiteList( _currentSetting );
