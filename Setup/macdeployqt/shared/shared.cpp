...
FrameworkInfo parseOtoolLibraryLine( const QString &line ) {
    ...
    if ( trimmed.startsWith( "/System/Library/" ) ||
            ...
            || trimmed.startsWith( "@loader_path" ) )
        return info;
    ...
    while ( part < parts.count() ) {
        ...
        if ( state == QtPath ) {
            ...
            if ( part < parts.count() && parts.at( part ).contains( ".dylib " ) ) {
                ...
                if ( currentPart.contains( ".framework" ) ) {
                    ...
                    qDebug() << "Assuming /usr/lib for " << trimmed;
        ...
        if ( state == DylibName ) {
            ...
            /* if ( info.frameworkDirectory == "" ) {
                 info.deployedInstallName = "@executable_path/" + info.frameworkName;
             } else { */
            ...
            /*   if ( info.frameworkDirectory == "" ) {
                   info.destinationDirectory = bundleBinaryDirectory + "/";
               } else { */
...
QString copyFramework( const FrameworkInfo &framework, const QString path ) {
    ...
    // qDebug() << "Target Framework Path: " << path;
...
DeploymentInfo deployQtFrameworks( const QString &appBundlePath, const QString &libraryName ) {
    ApplicationBundleInfo applicationBundle;
    applicationBundle.path = appBundlePath;
    applicationBundle.binaryPath =  appBundlePath  + "/Contents/MacOS/" + libraryName;
    return deployQtFrameworks( getQtFrameworks( applicationBundle.binaryPath ), applicationBundle.path, applicationBundle.binaryPath );
}
