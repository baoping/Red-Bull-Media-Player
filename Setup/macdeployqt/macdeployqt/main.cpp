...
int main( int argc, char **argv ) {
    ...
    if ( argc < 2 || appBundlePath.startsWith( "-" ) ) {
        qDebug() << "Usage: macdeployqt app-bundle [-no-plugins] [-dmg] [-extraLibs=lib1,lib2]";
        qDebug() << "";
    ...
    for ( int i = 2; i < argc; ++i ) {
        ...
        if ( argument.startsWith( "-extraLibs=" ) ) {
            QString libsStr = argument.replace( "-extraLibs=", "" );
            extraLibs = libsStr.split( ",", QString::SkipEmptyParts );
        }
    }
    ...
    if  ( ! extraLibs.isEmpty() ) {
        qDebug() << "Found extralibs arg: " << extraLibs.join( ", " );
        foreach( const QString & lib, extraLibs ) {
            deployQtFrameworks( appBundlePath, lib );
        }
    }
}
